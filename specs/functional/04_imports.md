...
the **Spoonacular API**, and the **persistent AI chat** (see `07_ai_chat.md`).
Every non-manual import passes through a shared preview/edit step before
becoming a saved recipe, and every imported recipe records its origin in the
recipe's *Source* field.
...
correct manually, or ask the persistent AI chat to refine the open form
(see `07_ai_chat.md`). Import does not fail because of enrichment.

## AI chat (creation)

New recipes can also be authored through the persistent AI chat (see
`07_ai_chat.md`). When a conversation in the chat sidebar produces a
recipe the user wants to save, the chat surfaces a **"Save as new
recipe"** action button. Clicking it sends the conversation to
`synthesizeRecipeFromMessages` (see `specs/tech/02_llm.md`) and lands
the user on the standard preview with **Source** ← `llm-chat`.
...
- **Chat transcript persistence**. Keeping the conversation attached to the
  recipe would allow re-prompting later. Out of scope for v1.
